...
class UIComponents {
  ...
  static renderSummaryCards(store) {
    ...
    const velocityBadge = velocityDiff >= 0 
      ? `<span class="badge-positive">+${store.currency}${velocityDiff.toLocaleString()} vs last month</span>`
    ...
    return `
      ...
      <div class="summary-card" style="--card-accent: var(--grad-accent);">
        ...
        <div class="summary-value">${progressPct.toFixed(1)}%</div>
  ...
  static renderPlanCard(plan, store) {
    ...
    if (analytics.status === 'completed') {
      statusBadge = `<span class="badge-tag" style="background: rgba(6,182,212,0.15); color: var(--accent-cyan);">🎉 GOAL COMPLETED</span>`;
    } else if (analytics.status === 'behind') {
      statusBadge = `<span class="badge-tag" style="background: rgba(245,158,11,0.15); color: var(--accent-amber);">⚠️ Behind Target</span>`;
    } else {
        ...
        <div>
          <div class="plan-card-header">
            ...
            <div class="plan-actions-menu">
              ${statusBadge}
              <button class="btn btn-outline btn-icon-only btn-sm" onclick="app.openEditPlanModal('${plan.id}')" title="Edit Plan">
              ...
              <button class="btn btn-outline btn-icon-only btn-sm" onclick="app.deletePlan('${plan.id}')" title="Delete Plan">
      ...
      return `
        <tr>
          <td colspan="6" style="text-align: center; padding: 2rem; color: var(--text-muted);">
            No savings records found. Click "+ Log Savings" to add your first entry!
          </td>
        </tr>
      `;
    }
    ...
    return transactions.map(tx => {
      ...
      return `
        <tr>
          ...
          <td><span class="badge-tag">${plan ? plan.category : 'General'}</span></td>
          <td>${typeBadge}</td>
